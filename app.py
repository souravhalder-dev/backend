...
try:
    ...
    print("Downloading punkt...", file=sys.stderr)
    nltk.download('punkt')

try:
    nltk.data.find('tokenizers/punkt_tab')
except LookupError:
    print("Downloading punkt_tab...", file=sys.stderr)
...
def get_wiki_content(title):
    ...
    try:
        ...
        # 'hatnote' and 'dablink' usually contain "For other uses, see..."
        unwanted_selectors = [
...
def summarize():
    ...
    print(f"Received request for title: {title}", file=sys.stderr)
